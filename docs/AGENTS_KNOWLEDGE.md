...
- **Season card import:** `python3 scripts/import_season_cards.py --year 2026 --apply --from-dir data/local_recovery/md_cards` parses operator betting cards (pre-tee canonical selection via `src/card_import.py`) into `picks`. Dry-run writes `data/local_recovery/manifest_YYYY.json`.
...
- **Season grading API:** `GET /api/grading/season?year=2026` — all `rounds` events for the year with dashboard/lab lane payloads.
