...
**Last verified:** 2026-08-31 on `main` (sports-desk tokens + remaining-route honesty). A corrupt `golf.db` must not take the process down. Lab never hydrates from Champion/production boards. Run `python3 -m pytest tests/` and `cd frontend && npm run test && npm run typecheck && npm run build` on PR.
...
│       ├── pages/            # Route-level screens: prediction, lab, compare, players, results, system, eval, 404
│       ├── lib/
...
2. **`GET /api/live-refresh/snapshot`** — JSON includes **`lab_upcoming_tournament`** and/or **`lab_live_tournament`** when the parallel lane is enabled and healthy. If **both** are `null`, Lab boards stay **empty** (amber “Lab lane off” banner). The SPA must **not** copy Champion/production `live_tournament` / `upcoming_tournament` onto `/lab`. **`diagnostics.lab_upcoming_state`** / **`lab_live_state`** may hint lane failures.
...
- `GET /api/tracks` — model-track registry: both active track config slots (`dashboard` champion, `lab` challenger) with stable `config_hash`, `effective_config_hash`, and activation history. Backed by `track_configs` + `src/track_registry.py`. Snapshot `strategy_meta.config_hash`/`track` and `picks.model_config_hash` carry the same provenance. Runtime precedence unchanged: env (`COCKPIT_SNAPSHOT_MODEL_VARIANT`) > registry seed > lab champion file > default.
- **Promotion workflow (flag-gated, `TRACK_PROMOTION_ENABLED`, default OFF):** `GET /api/tracks/promotion-readiness` (charter live-promotion gates + lab graded-sample gate); `POST /api/tracks/promote` `{reason, from_track?}` (records an auditable new active dashboard `track_configs` row with `parent_id` rollback chain + `evidence_json`); `POST /api/tracks/rollback` `{track?}` (restores the parent in one action). Promotion records the *config of record* + audit trail; making it the live runtime still requires the documented `COCKPIT_SNAPSHOT_MODEL_VARIANT`/profile change per `docs/research/LAB_PROMOTION_GATES.md` — no silent production swap. UI: `/eval` Promotion tab (gates + promote/rollback, requires typed confirmation).
- **Grading reconciliation:** `python3 scripts/grading_reconciliation.py [--source cockpit|lab_sandbox] [--write]` (`src/grading_reconciliation.py`) checks that every completed event graded its displayed +EV picks (no silently ungraded +EV picks; no orphan `pick_outcomes`). Exit code is non-zero on discrepancies. Run against the production DB on the VPS.
- **Season card import:** `python3 scripts/import_season_cards.py --year 2026 --apply --from-dir data/local_recovery/md_cards` parses operator betting cards (pre-tee canonical selection via `src/card_import.py`; `.md` preferred over `*_provenance_*.json` unless no parseable markdown). Default (no `--apply`) is dry-run and writes `data/local_recovery/manifest_YYYY_*.json`.
...
- `GET /api/players/photo-index` and `GET /api/players/{player_key}/photo` — cached PGA Tour headshots (`src/player_photos.py`, `player_photo_ids`). Seed with `scripts/seed_player_photo_ids.py`. Files live under `data/player_photos/`. Do not hotlink Cloudinary from the frontend.
...
- **Lab challenger shadow (H-6):** set `LAB_CHALLENGER_SHADOW_ENABLED=1` to register the promoted lab bundle as a `BaseModel` challenger (`src/models/lab_challenger.py`); it shadow-prices the same matchups the champion prices into `challenger_predictions` for live Brier/CLV (`GET /api/champion-challenger/summary`). Never touches live pricing; default OFF.
- **Trial-327 revalidation:** `python3 scripts/revalidate_trial327.py --db-path /opt/golf-model/data/golf.db --write` replays the promoted lab bundle on the pinned primary/holdout windows and writes a gate-checked dossier under `docs/research/experiments/`. The committed baseline is negative and the +13.8/+9.05 claim is unproven from committed artifacts, so the lab track is labelled "challenger (validation pending)" until this passes on production data.
...
- **Shell (sports desk):** `App.tsx` is intentionally thin; the real shell/router lives in `frontend/src/app/app-content.tsx`, which mounts `MonitoringShell` (`frontend/src/components/monitoring/monitoring-shell.tsx`), wires **Refresh** + command menu (Grade lives on **Results** only), and routes **`/`**, **`/lab`**, **`/players`**, **`/compare`**, **`/eval`**, **`/results`**, **`/system`**, **`/research/legacy-model`**, **`/research/champion-challenger`**, and **`/research/autoresearch`**. Unknown paths render `frontend/src/pages/not-found-page.tsx`. Primary nav stays **Dashboard / Lab / Compare / Players / Results / System**; Research includes Autoresearch. Legacy aliases redirect (`/grading`, `/track-record`, `/research/diagnostics`, `/matchups`, `/lab/picks`, `/cockpit-lab`). **`/compare`** (`frontend/src/pages/compare-page.tsx`) is a read-only same-event Dashboard-vs-Lab view. `LiveSnapshotProvider` owns live-refresh snapshot queries with `keepPreviousData`. Visual identity: sports-desk tokens in `frontend/src/styles/themes.css` + `desk.css` (loads last). Do not add another CSS overlay.
...
**Experiments / research:** `experiments`, `active_strategy`, `research_proposals`, `proposal_reviews`, `research_model_registry`, `live_model_registry`, `outlier_investigations`, `challenger_predictions` (champion-challenger shadow rows), `track_configs` (engine-scale: one active row per model track — `dashboard` champion / `lab` challenger — with canonical `strategy_bundle_json` + stable `config_hash`; read-only/provenance in Wave 1).
...
| Frontend dashboard (React SPA) | `frontend/src/App.tsx` (providers only) + `frontend/src/app/app-content.tsx` (shell, routes, Refresh, Lab honesty, dashboard/lab hydration) |
...
| Frontend route homes | `frontend/src/pages/` for route screens (incl. `not-found-page.tsx`); route map/redirects live in `frontend/src/app/app-content.tsx` |
