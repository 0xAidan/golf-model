...
- `GET /api/tracks` — model-track registry (engine-scale Wave 1, read-only): both active track config slots (`dashboard` champion, `lab` challenger) with stable `config_hash`, `effective_config_hash`, and activation history. Backed by `track_configs` + `src/track_registry.py`. Snapshot `strategy_meta.config_hash`/`track` and `picks.model_config_hash` carry the same provenance. Runtime precedence unchanged: env (`COCKPIT_SNAPSHOT_MODEL_VARIANT`) > registry seed > lab champion file > default.
