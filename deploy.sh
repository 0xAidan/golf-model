...
update_server() {
    ...
    ssh "$DEPLOY_HOST" bash << UPDATE_EOF
        ...
        # ``frontend/dist`` (entire tree: hashed chunk names change every build) and the
        # live snapshot are updated on the server; if they differ from the last commit,
        # a plain ``git pull`` aborts. Stash them — ``npm run build`` replaces dist, and
        # the snapshot is regenerated by the app.
