...
update_server() {
    ...
    ssh "$DEPLOY_HOST" bash << UPDATE_EOF
        ...
        # ``frontend/dist`` and the live snapshot are updated on the server; if they
        # differ from the last commit, a plain ``git pull`` aborts. Stash them —
        # ``npm run build`` replaces dist, and the snapshot is regenerated by the app.
        STASH_TS=\$(date -u +%Y%m%dT%H%M%SZ)
        git stash push -m "[deploy] auto-stash pre-pull \${STASH_TS}" -- frontend/dist/index.html data/live_refresh_snapshot.json || true
