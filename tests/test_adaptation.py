...
def test_aggregate_market_performance_empty(tmp_db):
    """No prediction_log entries -> empty dict."""
    result = aggregate_market_performance_for_tournament(99999)
    assert result == {}


def test_get_rolling_market_performance_no_data(tmp_db):
...
def test_adaptation_state_below_min_sample(tmp_db):
...
def test_check_recovery_no_data(tmp_db):
...
def test_log_ai_adjustment(tmp_db):
...
def test_evaluate_ai_adjustments_empty(tmp_db):
...
def test_get_ai_adjustment_config_default(tmp_db):
